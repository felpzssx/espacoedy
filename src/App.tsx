...
export default function App() {
    ...
    const eventTypes = [
        { icon: <Gift />, title: 'Aniversários', text: 'Festas infantis e adultos com toda infraestrutura necessária.' },
        { icon: <Heart />, title: 'Casamentos', text: 'O cenário perfeito para o seu "sim" e uma recepção inesquecível.' },
        { icon: <Gem />, title: 'Corporativo', text: 'Confraternizações e eventos empresariais em um ambiente exclusivo.' },
        { icon: <Users />, title: 'Eventos Familiares', text: 'Chás de bebê, batizados e almoços em família com total privacidade.' }
    ];

    const faqItems = [
        {
            q: 'Qual o horário de funcionamento para eventos?',
            a: 'Nossos eventos geralmente ocorrem das 09:00 às 22:00. Para horários diferenciados, consulte nossa equipe via WhatsApp.'
        },
        {
            q: 'O espaço possui limite de som?',
            a: 'Sim, seguimos as normas de silêncio locais. Som ambiente é permitido durante todo o evento, mas grandes estruturas de som devem ser validadas.'
        },
        {
            q: 'O estacionamento é incluso?',
            a: 'Sim, possuímos área para estacionamento interno e externo, garantindo segurança para você e seus convidados.'
        },
        {
            q: 'A piscina é aquecida?',
            a: 'Nossa piscina é natural, perfeita para os dias ensolarados da região. Ela passa por tratamento diário para garantir a máxima higiene.'
        },
        {
            q: 'É permitido levar animais de estimação?',
            a: 'Sim! Somos um espaço pet-friendly, desde que os pets estejam sob supervisão permanente dos donos.'
        }
    ];
    ...
    return (
        <div className="min-h-screen bg-brand-gray font-sans text-brand-dark overflow-x-hidden">
                        ...
                        <div className="flex justify-between items-center h-16 md:h-20">
                            ...
                            <div className="hidden lg:block">
                                <div className="ml-10 flex items-baseline space-x-6">
                                    ...
                                    <a href="#galeria" className={`transition-colors font-semibold text-sm uppercase tracking-wider ${isScrolled ? 'text-brand-dark hover:text-brand-pool' : 'text-white hover:text-brand-yellow'}`}>Galeria</a>
                                    <a href="#faq" className={`transition-colors font-semibold text-sm uppercase tracking-wider ${isScrolled ? 'text-brand-dark hover:text-brand-pool' : 'text-white hover:text-brand-yellow'}`}>FAQ</a>
                        ...
                        {isMenuOpen && (
                            ...
                            >
                                <div className="px-4 pt-2 pb-6 space-y-1">
                                    ...
                                    <a href="#galeria" onClick={() => setIsMenuOpen(false)} className="block px-3 py-3 rounded-xl text-base font-semibold hover:bg-brand-gray">Galeria</a>
                                    <a href="#faq" onClick={() => setIsMenuOpen(false)} className="block px-3 py-3 rounded-xl text-base font-semibold hover:bg-brand-gray">FAQ</a>
            ...
            <main>
                ...
                <section className="py-16 md:py-24 bg-brand-gray/50">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="text-center mb-16">
                            <span className="text-brand-pool font-bold text-xs uppercase tracking-widest mb-2 block">Versatilidade</span>
                            <h2 className="text-3xl md:text-4xl font-bold text-brand-dark mb-4 uppercase">O Espaço Ideal para Cada Momento</h2>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                            {eventTypes.map((type, index) => (
                                <motion.div
                                    key={index}
                                    whileHover={{ y: -10 }}
                                    className="bg-white p-8 rounded-[2rem] shadow-xl border border-brand-pool/5 text-center group transition-all"
                                >
                                    <div className="w-16 h-16 bg-brand-pool/10 rounded-2xl flex items-center justify-center text-brand-pool mx-auto mb-6 group-hover:bg-brand-pool group-hover:text-white transition-colors duration-500">
                                        {React.cloneElement(type.icon as React.ReactElement<{ className?: string }>, { className: "w-8 h-8" })}
                                    </div>
                                    <h3 className="text-xl font-bold text-brand-dark mb-3 uppercase tracking-tight">{type.title}</h3>
                                    <p className="text-gray-500 text-sm leading-relaxed">{type.text}</p>
                                </motion.div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
                            <div className="max-w-xl">
                                <h2 className="text-3xl md:text-4xl font-bold text-brand-dark mb-4 uppercase tracking-tight text-center md:text-left">Inspirado por Momentos Reais</h2>
                                <p className="text-gray-600 text-center md:text-left">Dê uma olhada em como o Espaço Edy se transforma para receber nossos clientes.</p>
                            </div>
                            <div className="flex gap-2 mx-auto md:mx-0">
                                <div className="p-3 rounded-full bg-brand-pool/10 text-brand-pool"><Zap className="w-5 h-5" /></div>
                            </div>
                        </div>
                ...
                <section id="faq" className="py-16 md:py-24 bg-white">
                    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="text-center mb-16">
                            <h2 className="text-3xl md:text-5xl font-black text-brand-dark mb-4 tracking-tighter uppercase">Tire suas <span className="text-brand-pool italic">Dúvidas</span></h2>
                            <p className="text-gray-500">Tudo o que você precisa saber antes de reservar o seu dia especial.</p>
                        </div>

                        <div className="space-y-4">
                            {faqItems.map((item, index) => (
                                <div key={index} className="border border-brand-pool/10 rounded-2xl md:rounded-3xl overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow">
                                    <button
                                        onClick={() => setActiveFaq(activeFaq === index ? null : index)}
                                        className="w-full flex items-center justify-between p-6 md:p-8 text-left outline-none transition-colors"
                                    >
                                        <span className={`text-[15px] md:text-lg font-bold ${activeFaq === index ? 'text-brand-pool' : 'text-brand-dark'}`}>{item.q}</span>
                                        <ChevronDown className={`w-5 h-5 text-brand-pool transition-transform duration-300 ${activeFaq === index ? 'rotate-180' : ''}`} />
                                    </button>
                                    <AnimatePresence>
                                        {activeFaq === index && (
                                            <motion.div
                                            ...
                                            >
                                                <div className="p-6 md:p-8 pt-0 border-t border-brand-gray">
            ...
            {/* Floating WhatsApp */}
            <motion.a
                href="https://wa.me/5511986209498"
                target="_blank"
                rel="noopener noreferrer"
                initial={{ scale: 0, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                whileHover={{ scale: 1.1 }}
                className="fixed bottom-6 right-6 z-[60] bg-green-500 text-white p-4 rounded-full shadow-2xl flex items-center justify-center group"
                aria-label="Fale conosco no WhatsApp"
            >
                <MessageCircle className="w-8 h-8" />
                <span className="absolute right-full mr-4 bg-white text-brand-dark px-4 py-2 rounded-xl text-sm font-bold shadow-xl opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                    Dúvidas? Chame aqui!
                </span>
                <span className="absolute inset-0 bg-green-500 rounded-full animate-ping opacity-20 -z-10" />
            </motion.a>
        </div>
    );
}
